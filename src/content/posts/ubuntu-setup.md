...
slug: ubuntu-setup
published: 2026-02-01 22:13:09
updated: 2026-07-26 09:04:34
description: Ubuntu Server 24.04/22.04 LTS 图文安装指南。涵盖镜像源优化、SSH 加固及性能调优，手把手教你打造高效、流畅的开发者运行环境。
image: https://img.olinl.com/file/post-img/ubuntu-setup/RcBwDYNQ.webp
category: Linux
tags:
  - 系统安装
draft: false
...
![网络配置](https://img.olinl.com/file/post-img/ubuntu-setup/CzNnyo7M.webp)
...
![用户配置](https://img.olinl.com/file/post-img/ubuntu-setup/QsO8TxfA.webp)
...
![硬盘配置](https://img.olinl.com/file/post-img/ubuntu-setup/s1E1NELm.webp)

可以看到 **/** 目录下才24G，但是上级却有48G，严重不对，现在要进行扩容

![lvm硬盘情况](https://img.olinl.com/file/post-img/ubuntu-setup/s0Yycwf2.webp)
...
## Ubuntu 24.04 vs 22.04 关键差异速查表

> 以下速查表专为习惯 22.04 的老用户整理，涵盖日常运维中**最容易踩坑**的差异点，帮助你快速切换到 24.04 的工作流。

| 差异维度 | Ubuntu 22.04 (Jammy) | Ubuntu 24.04 (Noble) | ⚠️ 过渡注意事项 |
| :--- | :--- | :--- | :--- |
| **Netplan 网络配置** | Netplan 配置目录：<br>`/etc/netplan/`<br>常用 `01-netcfg.yaml` | Netplan 1.0 默认启用<br>新增 `netplan status --diff` 查看变更 | 配置语法基本兼容，但建议用 `netplan status` 校验，旧版 `ifupdown` 已被完全替代 |
| **APT 软件源配置** | `/etc/apt/sources.list` 单文件 | **`/etc/apt/sources.list.d/ubuntu.sources`**<br>新格式（DEB822 格式） | ⚠️ **不要手动编辑 sources.list**，应使用 `ubuntu.sources`；添加第三方源需放到 `.sources` 文件，旧式 `.list` 仍兼容但推荐迁移 |
| **PHP 仓库** | `ppa:ondrej/php` 主流 | 官方支持 **PHP 8.3** 默认，8.4 可通过 `ppa:ondrej/php` 安装 | 注意 PHP-FPM 配置路径从 `/etc/php/8.1/` 变为 `/etc/php/8.3/` |
| **Node.js 安装** | 需手动添加 NodeSource 仓库 | 官方源内置 **Node.js 20 (LTS)** | 可通过 `apt install nodejs npm` 直接安装，无需额外仓库 |
| **Python 版本与包管理** | Python 3.10 + pip | **Python 3.12** 默认，`pip` 改用 `pipx` 推荐 | ⚠️ `apt install python3-pip` 不再直接提供 pip，建议用 `pipx` 或 `venv` |
| **服务管理 (systemd)** | systemd 249 | **systemd 255.4** | `journalctl` 输出格式优化，部分服务名称变更（如 `NetworkManager` 强化） |
| **防火墙与安全** | `ufw` 默认禁用 | `ufw` 默认禁用，但 **AppArmor 策略增强**（非特权用户命名空间限制） | 运行新容器或 Snap 应用时，可能需要调整 AppArmor 配置 |
| **桌面用户注意** | GNOME 42 + X11 默认 | **GNOME 46 + Wayland 默认** | 远程桌面（VNC/RDP）配置方式变化，Wayland 下部分截图/录屏工具需适配 |
| **软件安装来源** | Ubuntu Software（Snap 为主） | **新 App Center**（Snap + Deb 混合） | 推荐先 `apt search` 确认有 deb 包，或使用 `snap install` 替代 |
| **音频服务器** | PulseAudio | **PipeWire 默认** | 蓝牙音频、专业音频应用兼容性更好；若遇音频问题可回退 PulseAudio |
